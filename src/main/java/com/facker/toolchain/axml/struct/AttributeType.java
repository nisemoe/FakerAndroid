...
public class AttributeType {
	...
	private final StringPool mStringChunk;

	public AttributeType(StringPool mStringChunk) {
		this.mStringChunk = mStringChunk;
	}

	public String getAttributeData(AttributeData data){
		int rawType = data.type >> 24;
		if (rawType == ATTR_STRING) {
			return mStringChunk.getString(data.valueString);
		}
		if (rawType == ATTR_ATTRIBUTE) {
			return String.format("?%s%08X",getPackage(data.data),data.data);
		}
		if (rawType == ATTR_REFERENCE) {
			return String.format("@%s%08X",getPackage(data.data),data.data);
		}
		if (rawType == ATTR_FLOAT) {
			return String.valueOf(Float.intBitsToFloat(data.data));
		}
		if (rawType == ATTR_HEX) {
			return String.format("0x%08X",data.data);
		}
		if (rawType == ATTR_BOOLEAN) {
			return data.data!=0?"true":"false";
		}
		if (rawType == ATTR_DIMENSION) {
			return Float.toString(complexToFloat(data.data))+
				DIMENSION_UNITS[data.data & COMPLEX_UNIT_MASK];
		}
		if (rawType == ATTR_FRACTION) {
			return Float.toString(complexToFloat(data.data))+
				FRACTION_UNITS[data.data & COMPLEX_UNIT_MASK];
		}
		if (rawType >= ATTR_FIRSTCOLOR && rawType <= ATTR_LASTCOLOR) {
			return String.format("#%08X",data.data);
		}
		if (rawType >= ATTR_FIRSTINT && rawType <= ATTR_LASTINT) {
			return String.valueOf(data.data);
		}
		return String.format("<0x%X, type 0x%02X>",data.data, rawType);
	}
